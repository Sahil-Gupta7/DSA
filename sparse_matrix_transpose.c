//C Program to Find Transpose of Sparse Matrix
#include <stdio.h>
#include<stdlib.h>
int main()
{
    int x[100][100], transpose[100][100];            //Array Declaration
    ...
    for (i = 0; i < r; i++)
    {
        for (j = 0; j < c; j++)
        {
            ...
            if (x[i][j] == 0)      //Checking for Zero Elements
    ...
    for (i = 0; i < r; i++)
    {
        for (j = 0; j < c; j++)
        {
            printf("%d  ", x[i][j]); //Printing Matrix in Matrix Form
            if (j + 1 == c)
                printf("\n");
        }
    }
    //Checking If the Matrix is Sparse Matrix or Not
    if (z > t / 2)
    { //If the Matrix is Sparse Matrix
        printf("\n\tTOTAL No. of ELEMENTS : %d", t); //Printing Total No. of Elements in the Matrix
        printf(" \n\tNO. of NON-ZERO Elements in given Sparse Matrix : %d", p); //Printing No. of Non Zero Elements
        printf(" \n\tNO. of ZERO Elements in given Sparse Matrix : %d", z);     //Printing No. of Zero Elements
        ...
        for (i = 0; i < c; i++)
        {
            for (j = 0; j < r; j++)
            {
                printf("%d  ", transpose[i][j]); //Printing Transpose of the Matrix
            }
            printf("\n");
        }
